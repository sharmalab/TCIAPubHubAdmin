...
function findLatestVersion(doi, callback) {
  ...
  superagent
    .get(getVersionsForDoiURL + "?api_key=" + bindaas_api_key + "&doi=" + doi)
...
function postResourcesPayload(resource, doi, version, callback) {
  ...
  superagent
    .post(bindaas_addResourceURL + "?api_key=" + bindaas_api_key)
    .send(payload)
      ...
      async.each(
        ...
        function(resource, callback) {
          ...
          if (resource.type == "shared_list") {
            superagent
              .post("http://localhost:3001/api/createJNLP")
